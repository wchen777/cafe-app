...
export default function MyProfileView({ navigation }) {
  ...
  const postsComponents = userPosts?.map((p) => {
    switch (p.type) {
      ...
      case "Image":
        return (
          <ImageCard navigation={navigation} imagePost={p} key={count++} />
        );
      case "Audio":
        return (
          <AudioCard navigation={navigation} audioPost={p} key={count++} />
        );
      default:
        return;
    }
  ...
  return (
    <View style={{ flexDirection: "column", margin: 0, padding: 0 }}>
      <ScrollView
        style={{ marginBottom: 80, paddingTop: 15 }}
        refreshControl={
      ...
      >
        <View style={{backgroundColor: '#ffffff', marginHorizontal: 20}}>
          ...
          <View marginV-28 center row>
            ...
            >
              <Text color={Colors.orange30} marginR-20>
                <Text text60>
                  {!userData.following ? 0 : userData.following.length}
                </Text>
                <Text text70> Following </Text>
              </Text>
            </TouchableOpacity>

            <TouchableOpacity>
              <Text color={Colors.orange30} marginL-20>
                <Text text60>
                  {!userData.followers ? 0 : userData.followers.length}
                </Text>
        ...
        >
          {userData.portfolio !== "" && (
            <TouchableOpacity
              onPress={() => Linking.openURL(`https://${userData.portfolio}`)}
            >
        ...
        >
          {userData.twitter !== "" && (
            <TouchableOpacity
              onPress={() =>
                Linking.openURL(`https://twitter.com/${userData.twitter}`)
              }
            >
              <Text
                style={{ fontWeight: "bold", fontSize: 14, marginLeft: 14 }}
              >
                {" "}
          ...
          {userData.ig !== "" && (
            <TouchableOpacity
              onPress={() =>
                Linking.openURL(`https://instagram.com/${userData.ig}`)
              }
            >
              <Text
                style={{ fontWeight: "bold", fontSize: 14, marginLeft: 14 }}
